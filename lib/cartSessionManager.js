...
export async function detectExistingCartSession() {
  ...
  try {
    // Try to get cart from Shopify's cart.js endpoint
...
export async function createCompatibleCartSession(cartId) {
  ...
  try {
    ...
    // Try to set the cart session on Shopify's domain
    // This is a best-effort attempt to maintain session continuity
    const response = await fetch(`https://${SHOPIFY_DOMAIN}/cart/update.js`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
...
export async function validateCartSession(cartToken) {
  ...
  try {
    // Try to get cart data from Shopify
