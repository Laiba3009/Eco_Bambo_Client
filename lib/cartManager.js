...
class CartManager {
  ...
  async addToCart(variantId, quantity = 1) {
    ...
    try {
      ...
      // Trigger sync after cart update using direct Shopify URLs
      setTimeout(async () => {
        try {
          cartLogger.debug('CartManager', 'Triggering post-add sync via direct URL');
          
          // Use direct Shopify cart URL sync (most reliable method)
          const { directUrlSync } = await import('./directUrlSync');
          await directUrlSync.syncToShopifyCart();
